...
To address this, we try taking the the variables picked by the LASSO regression and fit a normal least squares regression on those variables:

```{r echo = FALSE}
model6A <- lm(violentPerPop ~ region + isUrban + pctWdiv + pctKids2Par + pctKidsBornNevrMarr + pctHousOccup + pctVacantBoarded + pctForeignBorn, data = USACrime2)
model6B <- lm(nonViolPerPop ~ region + isUrban + pctKids2Par + pctKidsBornNevrMarr + pctHousOccup + pctHousOwnerOccup + pctVacantBoarded + pctVacant6up + pctForeignBorn, data = USACrime2)
...
After completing the two model creation methods we now have to decide which models are better for each of the outcome variables. To do this we calculated the Mean Square Error for each of the models (Figure 12).  
...
plot(violentModelAICMin)
plot(nonViolModelAICMin)
```
